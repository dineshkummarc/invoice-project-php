...
function invoice_items_insert(&$error_message = '') {
	...
	if(function_exists('invoice_items_before_insert')) {
		$args = [];
		if(!invoice_items_before_insert($data, getMemberInfo(), $args)) {
	...
	if(Request::val('filterer_invoice')) {
		sql("UPDATE `invoice_items` SET `invoice`='" . makeSafe(Request::val('filterer_invoice')) . "' WHERE `id`='" . makeSafe($recID, false) . "'", $eo);
	}

	update_calc_fields('invoice_items', $recID, calculated_fields()['invoice_items']);
	...
	if(strlen(Request::val('SelectedID'))) invoice_items_copy_children($recID, Request::val('SelectedID'));
...
function invoice_items_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	<script>
		...
		AppGini.current_invoice__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['invoice'] : htmlspecialchars($filterer_invoice, ENT_QUOTES)); ?>"};
	...
	if($dvprint) {
		...
		$templateCode .= "\t\t\tcomplete: function() { " . (($arrPerm[1] || (($arrPerm[3] == 1 && $ownerMemberID == getLoggedMemberID()) || ($arrPerm[3] == 2 && $ownerGroupID == getLoggedGroupID()) || $arrPerm[3] == 3)) ? "\$j('#item$rnd1').prop('disabled', false); " : "\$j('#item$rnd1').prop('disabled', true); ")." \$j(window).resize(); }\n";
	}
	$templateCode .= "\t\t});\n";
	$templateCode .= "\t};\n";
